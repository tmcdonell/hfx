...
#if 0
bIonLadder, yIonLadder :: Peptide -> IO [CFloat]
bIonLadder p  = G.forceEither `fmap` G.peekArrayAt (offset p) len (bions (parent p))
  where (m,n) = terminals p
        len   = fromIntegral (n-m)

yIonLadder p  = G.forceEither `fmap` G.peekArrayAt (1 + offset p) len (yions (parent p))
  where (m,n) = terminals p
        len   = fromIntegral (n-m)
#endif
...
readFasta fasta = do
    ...
    return   $  map (\(S.Seq h d _) -> Protein h d undefined []) database
...
--  - Perform on-device filtering with map and compact/bpermute? (environments)
--  - Implicit allocation?
--
...
digestProtein cp protein =
  ...
  -- Permute ion indices
  --
  replicate_ui ion_idx 1 num_ions
  permute_ui fst_idx ion_idx offsets num_seqs
  scanl1Seg_plusui ion_idx flags ion_idx num_ions

  --
  -- Convert the amino acid code into the corresponding monoisotopic mass
  --
  bpermute_ui seq_ions all_ions ion_idx num_ions
  map_getAAMass all_ions ion_masses num_ions

  --
  -- The mass ladder generated by successive breaks of the peptide backbone from
  -- the N- to C- terminus (y-ions). Also extract the total residual mass of
  -- each sequence.
  --
  scanr1Seg_plusf ion_masses flags ladder num_ions
  bpermute_f ladder residuals offsets num_seqs
  ...
  where
    ...
    idx1      = map fromIntegral . fst . unzip $ splices

    lengths   = map (\(m,n) -> fromIntegral (n - m + 1)) splices
    num_ions  = fromIntegral $ sum lengths
    ions      = map fromIntegral (W.unpack (seqdata protein))
